...
export default function SearchPage() {
  ...
  const [isSearched, setIsSearched] = useState(false); // ✅ 검색 버튼 누른 적 있는지 여부

  const handleSearch = async () => {
    if (!query.trim()) return;
    setLoading(true);
    setIsSearched(true); // ✅ 검색 시작 표시
  ...
  return (
    ...
    >
      ...
      <div className="max-w-3xl mx-auto py-10 px-4">
        <div
          className="w-full flex items-center border border-[#E5E5E5] rounded-full overflow-hidden transition-all duration-300 px-4 py-2 gap-2
